...
public class RecyclerAdapterWithHF extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<View> mCarouse = new ArrayList<View>();//淇濆瓨杞挱View
    ...
    public final void onBindViewHolder(final RecyclerView.ViewHolder vh, int position){
        ...
        }else if(mCarouse.size()>0&&position==mHeaders.size()){//杩欎釜鏃跺�檓Headers.size()鍊间负0
//            System.out.println("鏈夊灏戜釜澶碫iew:"+mHeaders.size()+"鍊肩瓑浜庡灏�:"+(mHeaders.size()-1));
            View v = mCarouse.get(mHeaders.size());//鍙栧嚭杞挱鐨刅iew
            prepareHeaderFooter((HeaderFooterViewHolder) vh, v);
    ...
    public final int getItemViewType(int position) {
        ...
        } else if (mCarouse.size()>0&&mHeaders.size()==position){ //鍒ゆ柇闆嗗悎涓暟&&position==0  杩欎釜鏃跺�檓Headers閲岄潰鏄病鏈夊�肩殑
            return TYPE_CAROUSE;
        }else if (isFooter(position)) {
            return TYPE_FOOTER;
        }
    ...
    private class MyOnClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            int position = getRealPosition(vh.getPosition());
